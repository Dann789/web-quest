...
/**
 * Dosen Leaderboard Page - Menampilkan ranking mahasiswa
 */
export default function DosenLeaderboardPage() {
  const { user } = useAuth();
  ...
  const leaderboardData = [
    ...
    { rank: 5, username: 'StudentUser', xp: 2400, time: '18h 20m', avatar: 'ME', isMe: false },
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-muted-foreground mt-2">
            Monitoring peringkat mahasiswa
          </p>
        </div>
        ...
        <div className="order-1 md:order-2 flex flex-col items-center z-10 scale-110 md:mb-8">
          ...
          <Card className="w-full bg-gradient-to-t from-amber-500/10 to-transparent border-t-4 border-t-amber-400 mt-2 shadow-lg">
            <CardContent className="p-6 text-center">
              ...
              <p className="text-sm text-muted-foreground mt-1" title="Total Waktu">⏱ {topThree[0].time}</p>
            </CardContent>
          </Card>
        </div>
          ...
          >
            ...
            <div className="ml-4 flex-1">
              <p className={`font-bold ${player.isMe ? 'text-primary' : ''}`}>
                {player.username} {player.isMe && '(User)'}
              </p>
            </div>
            ...
            <div className="font-mono font-bold text-lg w-20 text-right">{player.xp} XP</div>
          </div>
        ))}
      </div>
