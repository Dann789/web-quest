...
export const levelRoutes = new Elysia({ prefix: "/api/levels" })
  // GET dashboard stats (harus di atas /:id agar tidak bentrok dengan param)
  .get("/stats", () => LevelController.getDashboardStats())
  ...
  .post(
    "/",
    ({ body }) => LevelController.createLevel(body as CreateLevelRequest),
    {
      body: t.Object({
        name: t.String({
          minLength: 1,
  ...
  .patch(
    "/:id",
    ({ params: { id }, body }) =>
      LevelController.updateLevel(id, body as UpdateLevelRequest),
    {
      params: t.Object({
        id: t.Numeric(),
      }),
  ...
  .get("/completion", () => LevelController.getLevelCompletion());
